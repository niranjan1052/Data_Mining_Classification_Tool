...
public class ridge_hashmap {
   ...
   //stores the TF*IDF representation of the document vectors
	...
	//	outputfile = "classification_colution";
		...
		ridge_hashmap ridgeobj = new ridge_hashmap();
		...
		LinkedHashMap<Integer,Double> xdotw = new LinkedHashMap<Integer,Double>(); 
		 ...
		 for( String classname : label_docids.keySet()){
			...
			for( Map.Entry<Integer,Double> m : xdotw.entrySet()){
				if(m.getKey()<500)
				System.out.println(" row "+ m.getKey() + " - y value  "+ m.getValue());
			}
	...
	public  void calc_w_regression()
	{
	   ...
	   for( String classname : label_docids.keySet())
	   {
			...
			for( iterationcount =0;iterationcount<60;iterationcount++)
			{
				  featurecount=0;
			     for(int i=1;i<=feature_labels.size();i++ )
				  ...
				  for(Map.Entry<Integer, Double> m : W.entrySet()){
				   ...
				   System.out.println("change in error is "+ (Math.abs(total_error-prev_error)));
				   if(Math.abs(prev_error-total_error)<0.01){
					  ...
					  System.out.println(" no of non zero values in w " + nonzerow);
				    	  break;
				    }
